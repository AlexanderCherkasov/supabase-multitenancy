...
select ok(
  (select pg_get_functiondef(oid) ilike '%not p.is_deprecated%' from pg_proc where proname='access_level' and pronamespace='multitenancy'::regnamespace),
  'deprecated permissions deny'
);
select ok(
  (select pg_get_functiondef(oid) ilike '%m.status = ''active''%' from pg_proc where proname='access_level' and pronamespace='multitenancy'::regnamespace),
  'only active memberships authorize'
);
select ok(
  (select pg_get_functiondef(oid) ilike '%cardinality(p_scope_ids) = 0%' from pg_proc where proname='access_level' and pronamespace='multitenancy'::regnamespace),
  'empty scopes require tenant-wide assignment'
);
select ok(
  (select pg_get_functiondef(oid) ilike '%least(v_effective_rank, v_rank)%' from pg_proc where proname='access_level' and pronamespace='multitenancy'::regnamespace),
  'multi-scope access uses the weakest covered level'
);
select ok(
  (select pg_get_functiondef(oid) ilike '%ra.scope_id is null or ra.scope_id = v_scope_id%' from pg_proc where proname='access_level' and pronamespace='multitenancy'::regnamespace),
