...
select ok(
  (select pg_get_functiondef(oid) ilike '%search_path = ''''' from pg_proc where proname='handle_new_user' and pronamespace='multitenancy'::regnamespace),
...
select ok(
  (select pg_get_constraintdef(oid) ilike '%1 and 720%' from pg_constraint where conname like '%invitation_ttl%'),
...
select is(
  (select count(*)::int from pg_proc p join pg_namespace n on n.oid=p.pronamespace
   where n.nspname='multitenancy' and p.prosecdef and pg_get_functiondef(p.oid) not ilike '%search_path = ''''%' ),
  0,
