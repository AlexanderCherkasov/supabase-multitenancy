...
select ok(
  (select array_agg(grantee::text) from information_schema.routine_privileges where routine_schema='multitenancy' and routine_name='invitation_preview') @> array['anon'],
  'invitation_preview granted to anon'
);
select ok(
  (select count(*)::int from information_schema.routine_privileges where routine_schema='multitenancy' and routine_name='accept_invitation' and grantee='anon') = 0,
  'accept_invitation not granted to anon'
);
