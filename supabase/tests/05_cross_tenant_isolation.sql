...
create policy "sample_docs select scoped" on public.sample_docs for select to authenticated using (api.has_access(tenant_id, 'documents.read', array[project_scope_id], 'all'));
drop policy if exists "sample_docs insert scoped" on public.sample_docs;
create policy "sample_docs insert scoped" on public.sample_docs for insert to authenticated with check (api.has_access(tenant_id, 'documents.create', array[project_scope_id], 'all'));
drop policy if exists "sample_docs update scoped" on public.sample_docs;
create policy "sample_docs update scoped" on public.sample_docs for update to authenticated using (api.has_access(tenant_id, 'documents.update', array[project_scope_id], 'all')) with check (api.has_access(tenant_id, 'documents.update', array[project_scope_id], 'all'));
drop policy if exists "sample_docs delete scoped" on public.sample_docs;
create policy "sample_docs delete scoped" on public.sample_docs for delete to authenticated using (api.has_access(tenant_id, 'documents.delete', array[project_scope_id], 'all'));

-- verify policies exist and use authorize
select ok(
  (select count(*)::int from pg_policies where schemaname='public' and tablename='sample_docs' and coalesce(qual, with_check) ilike '%api.has_access%') >= 4,
  'sample_docs has 4 policies using api.has_access'
);
...
select ok(
  api.authorize('00000000-0000-0000-0000-000000000000'::uuid, 'documents.read', null) = false,
  'authorize denies unknown tenant'
);
select ok(
  api.authorize('00000000-0000-0000-0000-000000000000'::uuid, 'unknown.permission', null) = false,
