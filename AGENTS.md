...
1. **Schema Encapsulation**: Package tables and implementation RPCs reside exclusively in private `multitenancy`; browser RPCs and RLS helpers are called through exposed `api` wrappers.
2. **Access Model**: Role permissions use `'own'` or `'all'`. Do not store SQL function names in role tables.
3. **Immutability**: Always protect tenant and scope columns on application tables with `api.enforce_protected_keys_immutable()`.
4. **Skills & References**:
